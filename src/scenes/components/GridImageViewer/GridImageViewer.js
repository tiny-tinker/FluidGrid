...
class GridImageViewer extends React.Component {
    ...
    gridImage(options) {
        ...
        // width of the area
        const padding = parseFloat(albumContainerStyle.getPropertyValue('padding-left'));
        let defaults = {
        ...
        settings.images.each( function(index) {
            ...
            let w = (typeof $img.data("width") != 'undefined') ? $img.data("width") : $img.width(),
    ...
    renderImages(imageUrls) {
        return imageUrls.map(item=> {
            item.width  /= 10;
            item.height /= 10;
                return (<Image imgSrc={'https://picsum.photos/'+item.width+'/'+item.height+'?image='+item.id} dataWidth={item.width} dataHeight={item.height}/>);
        });
    }
    ...
    render() {
        ...
        return(
            <div className="grid-image-viewer" ref={(ref) => this.albumContainer = ref}>
